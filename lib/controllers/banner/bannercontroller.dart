...
class BannerController extends GetxController {
  ...
  Future<bool> applyLoyaltyPoints(int amount) async {
    try {
      ...
      if (res.hasException) {
        debugPrint('[BannerController] ApplyLoyaltyPoints Exception: ${res.exception}');
        utilityController.setLoadingState(false);
        return false;
      }
  ...
  Future<bool> removeLoyaltyPoints() async {
    try {
      ...
      final res = await GraphqlService.client.value.mutate$RemoveLoyaltyPointsFromActiveOrder(
        Options$Mutation$RemoveLoyaltyPointsFromActiveOrder(),
      );

      if (res.hasException) {
        debugPrint('[BannerController] RemoveLoyaltyPoints Exception: ${res.exception}');
        utilityController.setLoadingState(false);
        return false;
      }
  ...
  List<Map<String, dynamic>> getCouponProducts(String couponCode) {
    try {
      ...
      for (final action in coupon.actions) {
        ...
        if (action.code == 'add_products' || action.code == 'contains_products') {
            ...
            if (arg.name == 'productVariantIds') {
              ...
              if (arg.value is List) {
                ...
                if (stringValue.startsWith('[') && stringValue.endsWith(']')) {
                  ...
                  final cleanString = stringValue.substring(1, stringValue.length - 1);
                  variantIds = cleanString.split(',').map((e) => e.trim()).toList();
                  debugPrint('[BannerController] Parsed variant IDs from brackets: $variantIds');
                } else if (stringValue.contains(',')) {
                  // Split by comma
                  variantIds = stringValue.split(',').map((e) => e.trim()).toList();
              ...
              for (final product in productsList) {
                if (product is Map<String, dynamic>) {
                  products.add({
                    ...
                    'name': product['name']?.toString() ?? 'Product from ${coupon.name}',
                    'productVariantId': product['variantId']?.toString() ?? product['id']?.toString() ?? 'unknown',
                    'price': (product['price'] as num?)?.toDouble() ?? 0.0,
                    'priceWithTax': (product['priceWithTax'] as num?)?.toDouble() ?? 0.0,
  ...
  Future<void> getCouponCodeList() async {
    try {
      ...
      if (res.hasException) {
        debugPrint('[BannerController] ❌ GetCouponCodeList Exception: ${res.exception}');
        debugPrint('[BannerController] Exception details: ${res.exception?.graphqlErrors}');
        debugPrint('[BannerController] Exception link: ${res.exception?.linkException}');
        utilityController.setLoadingState(false);
        return;
      }
      ...
      if (couponData != null) {
        ...
        couponCodesLoaded.value = true;
          ...
          for (int i = 0; i < fetchedCoupons.length; i++) {
            ...
            debugPrint('[BannerController] - Description: ${coupon.description}');
  ...
  Future<Map<String, dynamic>> _validateMinimumOrderAmount(CouponCodeModel coupon) async {
    try {
          ...
          for (final arg in condition.args) {
            ...
            if (arg.name == 'amount') {
              ...
              if (cartTotal < requiredAmount) {
                return {
                  'valid': false,
                  'message': 'Minimum order amount of ${_formatPrice(requiredAmount.toInt())} required. Current cart total is ${_formatPrice(cartTotal.toInt())}.',
                  'error': 'MINIMUM_ORDER_AMOUNT_NOT_MET',
      ...
      return {
        'valid': true,
        'message': 'Minimum order amount requirement met'
      };
    } catch (e) {
  ...
  String _formatPrice(int priceInCents) {
    // Convert cents to rupees (assuming 100 cents = 1 rupee)
    final rupees = priceInCents / 100;
  ...
  Future<Map<String, dynamic>> applyCouponCode(String couponCode) async {
    try {
      ...
      if (res.hasException) {
        debugPrint('[BannerController] ApplyCouponCode Exception: ${res.exception}');
        utilityController.setLoadingState(false);
        return {
          'success': false,
  ...
  Future<bool> removeCouponProducts(String couponCode) async {
    try {
      ...
      debugPrint('[BannerController] Found ${productVariantIds.length} products to remove for coupon: $couponCode');
      debugPrint('[BannerController] Product variant IDs to remove: $productVariantIds');
      ...
      debugPrint('[BannerController] Removed $removedCount out of ${productVariantIds.length} products');
      ...
      if (removedCount > 0) {
        ...
        if (updatedCart != null) {
          ...
          debugPrint('[BannerController] Cart after removal has ${updatedLines.length} lines');
        }
      }
  ...
  Future<bool> _removeOrderLineById(String orderLineId) async {
    try {
      ...
      if (res.hasException) {
        debugPrint('[BannerController] ✗ GraphQL exception removing order line $orderLineId: ${res.exception}');
        return false;
      }
  ...
  Future<bool> removeCouponCode(String couponCode) async {
    try {
      ...
      debugPrint('[BannerController] Removing products added by coupon: $couponCode');
      final productsRemoved = await removeCouponProducts(couponCode);
      if (!productsRemoved) {
        debugPrint('[BannerController] Warning: Failed to remove some products for coupon $couponCode');
      }

      final res = await GraphqlService.client.value.mutate$RemoveCouponCode(
        Options$Mutation$RemoveCouponCode(
          variables: Variables$Mutation$RemoveCouponCode(couponCode: couponCode),
        ),
      );

      if (res.hasException) {
        debugPrint('[BannerController] RemoveCouponCode Exception: ${res.exception}');
        utilityController.setLoadingState(false);
        return false;
      }
  ...
  Future<Map<String, dynamic>> toggleCouponCode(String couponCode) async {
    if (appliedCouponCodes.contains(couponCode)) {
      ...
      return {
        'success': success,
        'message': success ? 'Coupon code removed successfully' : 'Failed to remove coupon code',
        'action': 'removed'
      };
    } else {
  ...
  Future<Map<String, dynamic>> getCouponValidationStatus(String couponCode) async {
  ...
  Future<Map<String, dynamic>> addCouponProductsToCart(String couponCode) async {
    try {
      ...
      for (int i = 0; i < couponProducts.length; i++) {
        ...
        debugPrint('[BannerController] Product $i: ${product['name']} (Variant ID: ${product['productVariantId']}, Qty: ${product['quantity']})');
      }
      ...
      for (final product in couponProducts) {
        try {
          ...
          if (res.hasException) {
            debugPrint('[BannerController] Network error adding product $productName: ${res.exception}');
            failedProducts.add({
              'product': productName,
              'error': 'Network error: ${res.exception}'
            });
            continue;
          }
          ...
          if (result != null) {
            ...
            } else if (result is Mutation$AddToCart$addItemToOrder$$InsufficientStockError) {
      ...
      if (failedProducts.isNotEmpty) {
        ...
        for (final product in failedProducts) {
          debugPrint('[BannerController] - ${product['product']}: ${product['error']}');
        }
      }
  ...
  Future<Map<String, dynamic>> applyCouponCodeWithProducts(String couponCode) async {
    try {
      ...
      try {
        final rollbackResult = await _rollbackAddedProducts(couponCode);
        debugPrint('[BannerController] Rollback result: ${rollbackResult['success']}');
      } catch (rollbackError) {
  ...
  Future<Map<String, dynamic>> _rollbackAddedProducts(String couponCode) async {
    try {
      ...
      final productVariantIds = couponAddedProducts[couponCode]!;
      debugPrint('[BannerController] Rolling back ${productVariantIds.length} products for coupon: $couponCode');
      debugPrint('[BannerController] Product variant IDs to remove: $productVariantIds');
      ...
      debugPrint('[BannerController] Rollback completed: Removed $removedCount out of ${productVariantIds.length} products');
