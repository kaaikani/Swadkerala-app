  ...
  static Future<void> testSimDetection(BuildContext context) async {
    ...
    if (hasPermission) {
      ...
      for (int i = 0; i < simInfoList.length; i++) {
        ...
        debugPrint(
            '[SimTest] SIM $i: ${simInfo.phoneNumber.isEmpty ? "No number" : simInfo.last10Digits} (${simInfo.carrierName}) [Active: ${simInfo.isActive}]');
      }
      ...
      if (simInfoList.length > 1) {
        ...
        if (selectedSim != null) {
          debugPrint(
              '[SimTest] Selected SIM: ${selectedSim.phoneNumber.isEmpty ? "No number" : selectedSim.last10Digits}');
        } else {
